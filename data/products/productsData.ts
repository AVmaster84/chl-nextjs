export const productsData = [
  {
    id: 1,
    name: "Apple Watch Series 9 [GPS 45mm] Smartwatch with Midnight Aluminum Case",
    category: "Watches",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "WHY APPLE WATCH SERIES 9 — Your essential companion for a healthy life is now even more powerful. The S9 chip enables a super-bright display and a magical new way to quickly and easily interact with your Apple Watch without touching the screen. Advanced health, safety and activity features provide powerful insights and help when you need it. And redesigned apps in watchOS give you more information at a glance.",
      "CARBON NEUTRAL — An aluminium Apple Watch Series 9 paired with the latest Sport Loop is carbon neutral.",
      "ADVANCED HEALTH FEATURES—Keep an eye on your blood oxygen. Take an ECG anytime. Get notifications if you have an irregular heart rhythm. See how much time you spent in REM, Core, or Deep sleep with sleep stages. Temperature sensing provides insights into overall well-being and cycle tracking. And take note of your state of mind to help build emotional awareness and resilience."
    ],
    price: 400,
    discount: 10,
    rating: 4.5,
    stockItems: 5,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        rating:4,
        author: "Shohag miah",
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    brand: "Apple",
    color: ['white', 'gray', 'blue','silver'],
    images: [
      "/images/products/apple-watch-9-removebg-preview.png",
      "/images/products/apple-watch-9-3-removebg-preview.png",
    ],
  },
  {
    id: 2,
    name: "Apple Watch SE (2nd Gen) [GPS 44 mm] Smart Watch w/Starlight Aluminium Case & Starllight Sport Band",
    category: "Watches",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 300,
    brand: "Apple",
    color: ['white','gray', 'black', 'red'],
    rating: 4.3,
    discount: 10,
    stockItems: 10,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        rating:4,
        author: "Shohag miah",
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    images: [
      "/images/products/apple-watch-se-removebg-preview.png",
      "/images/products/apple-watch-se-2-removebg-preview.png",
    ],
  },
  {
    id: 3,
    name: "FireBolt Ninja SmartWatch | lorem ipsum doller",
    category: "Watches",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 200,
    rating: 4,
    brand: "Firebolt",
    color: ['black','gray', 'yellow'],
    material: "Aluminum",
    stockItems: 3,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        rating:4,
        author: "Shohag miah",
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 10,
    images: [
      "/images/products/firebolt-ninja-removebg-preview.png"
    ],
  },
  {
    id:4,
    name: "Samsung Galaxy 4 Smartwatch | lorem ipsun doller",
    category: "Watches",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!.",
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!, your Lock Screen stays glanceable, so you don’t have to tap it to stay in the know",
    ],
    price: 400,
    discount: 10,
    rating: 4.1,
    stockItems: 5,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    brand: "none",
    color: ['white', 'gray', 'blue'],
    images: [
      "/images/products/galaxy-watch-4-removebg-preview.png",
      "/images/products/galaxy-watch-4-2-removebg-preview.png",
    ],
  },
  {
    id:5,
    name: "Asus Vivobook Pro | lorem ipsum",
    category: "Computers",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 3300,
    brand: "Asus",
    color: ['white','gray', 'black', 'red'],
    material: "Aluminum",
    rating: 4,
    discount: 10,
    stockItems: 8,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    images: [
      "/images/products/asus-vivobook-removebg-preview.png",
      "/images/products/asus-vivobook-2-removebg-preview.png",
    ],
  },
  {
    id: 6,
    name: "Dell Gaming Laptop | lorem ipsum dlllres",
    category: "Computers",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 1000,
    rating: 4.2,
    brand: "Dell",
    color: ['black','gray', 'yellow'],
    stockItems: 3,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 10,
    images: [
      "/images/products/dell-gaming-removebg-preview.png"
    ],
  },
  {
    id: 7,
    name: "Lenova Powerfull Laptop | lorem ipsum dlllres",
    category: "Computers",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 800,
    rating: 4.2,
    brand: "Lenova",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 3,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/lenova-removebg-preview.png",
      '/images/products/lenova-2-removebg-preview.png'
    ],
  },
  {
    id: 8,
    name: "MSI Modern Laptop 14 | lorem ipsum dlllres",
    category: "Computers",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 900,
    rating: 4.2,
    brand: "MSI",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 3,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 6,
    images: [
      "/images/products/msi-modern-14-removebg-preview.png",
      '/images/products/msi-modern-14-2-removebg-preview.png',
      '/images/products/msi-modern-14-3-removebg-preview.png'
    ],
  },
  {
    id: 9,
    name: "Sony Dynamic Wh High Headphone | lorem ipsum dlllres",
    category: "Headphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 300,
    rating: 4.2,
    brand: "Sony",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 17,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        rating:4,
        author: "Shohag miah",
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 15,
    images: [
      "/images/products/sony-dynamic-removebg-preview.png",
      '/images/products/sony-dynamic-2-removebg-preview.png',
    ],
  },
  {
    id: 10,
    name: "Sony Normal Wh High Headphone | lorem ipsum dlllres",
    category: "Headphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 250,
    rating: 3.5,
    brand: "Sony",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 15,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/song-wh-removebg-preview.png",
    ],
  },
  {
    id: 11,
    name: "Seiheimer 12 High Headphone | lorem ipsum dlllres",
    category: "Headphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 200,
    rating: 3.5,
    brand: "Sony",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 0,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 12,
    images: [
      "/images/products/senheiser-removebg-preview.png",
    ],
  },
  {
    id: 12,
    name: "Prothonus Noise Cancalling Headphone | lorem ipsum dlllres",
    category: "Headphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 210,
    rating: 3.8,
    brand: "Prothonus",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 5,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 12,
    images: [
      "/images/products/prothonics-removebg-preview.png",
    ],
  },
  {
    id: 13,
    name: " Peco M6 Smartphone | lorem ipsum dlllres",
    category: "Smartphones",
    description:
    "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 500,
    rating: 3.8,
    brand: "Peco",
    color: ['black','gray', 'yello','blue'],
    stockItems: 5,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 12,
    images: [
      "/images/products/peco-m6-removebg-preview.png",
      '/images/products/peco-m6-2-removebg-preview.png'
    ],
  },
  {
    id: 14,
    name: "Realme C 13 Smartphone | lorem ipsum doller",
    category: "Smartphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 600,
    rating: 4.8,
    brand: "Realme",
    color: ['black','gray', 'yello','blue'],
    stockItems: 5,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 12,
    images: [
      "/images/products/readme-13-c-removebg-preview.png",
      '/images/products/readme-13c-2-removebg-preview.png'
    ],
  },
  {
    id: 15,
    name: "Lava Agni Smartphone | lorem ipsum doller",
    category: "Smartphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 500,
    rating: 4.5,
    brand: "Realme",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 0,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        rating:4,
        image: "/images/people/person.jpg",
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/lava_agni-removebg-preview.png",
    ],
  },
  {
    id: 16,
    name: "Samsung Galaxy 15 Pro Smartphone | lorem ipsum doller",
    category: "Smartphones",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium quas reiciendis quae, enim nesciunt, illo sit ea aspernatur quo perferendis! Cumque fuga impedit corrupti!",
    aboutItem: [
      "Tangent Impulse Open-ear wireless headphones are more comfortable to wear for long hours, as they do not create any pressure in the ears | Ultra-Lightweight, just weighing 16 gms.",
      "SafeBeats Design, these Bluetooth earphones rest gently on the outer ear, relieving pressure & preventing damage to the eardrums | Beware of your surroundings while Listening to Music",
      "Large 13mm Dynamic Drivers for Immersive Stereo Sound Experience | Enjoy music for up to 10 hours on a single charge",
      "Bluetooth V5.3 with 1-Step Pairing, 10 meters Strong Wireless Connectivity & Dual-Device Pairing | Access Your Phone`s Voice Assistant Instantly thru Tangent Impulse | Touch Control in Right Earbud",
    ],
    price: 800,
    rating: 4.9,
    brand: "Samsung",
    color: ['black','gray', 'yellow','blue'],
    stockItems: 0,
    reviews: [
      {
        content: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Distinctio voluptatem aliquam reprehenderit debitis quidem accusantium",
        author: "Shohag miah",
        image: "/images/people/person.jpg",
        rating:4,
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/galaxy-15-removebg-preview.png",
    ],
  },
  {
    id: 17,
    name: "Camera IP 4MP Dahua DH-IPC-HFW2449M-S-B-PRO",
    category: "Camera IP",
    description: [
      "DAHUA DH-IPC-HFW2449M-S-B-PRO là camera quan sát IP chính hãng, thiết kế dạng bán cầu, với công nghệ hồng ngoại thông minh. Camera hỗ trợ hỗ trợ ánh sáng kép thông minh cùng tầm quan sát xa 50 mét, hỗ trợ tính năng phát hiện thông minh, tích hợp míc. Hỗ trợ chống bụi nước IP67.",
      "Với công nghệ AI-ISP kết hợp cảm biến lớn, camera DAHUA DH-IPC-HFW2449M-S-B-PRO cho hình ảnh vượt trội ban đêm cũng như ban ngày. Trải nghiệm công nghệ WizColor mới nhất của Dahua, hình ảnh ban đêm rực rỡ như ban ngày. Camera thiết kế chắc chắn, vỏ kim loại, kèm chân đế. Camera phù hợp lắp đặt camera cho gia đình, cửa hàng, văn phòng, trung tâm thương mại, nhà xưởng, …"],
    aboutItem: [
      '– Độ phân giải 4 Megapixel cảm biến CMOS kích thước 1/1.8”',
      '– 2688 × 1520@(1–20 fps)/ 2560 ×1440@(1–25/30 fps)',
      '– Hỗ trợ Starlight với độ nhạy sáng siêu thấp 0.0005 lux@F1.0',
      '– Công nghệ AI-ISP kết hợp cảm biến lớn cho hình ảnh vượt trội ban đêm cũng như ban ngày',
      '– Chuẩn nén H265+',
      '– Ống kính cố định 3.6mm góc nhìn rộng 94° ( 6mm)',
      '– Tầm xa LED trắng thu hình có màu 50m',
      '– Chống ngược sáng WDR (120dB)',
      '– Chế độ ngày đêm (ICR), tự động cân bằng trắng (AWB), tự động bù sáng (AGC), chống ngược sáng(BLC), chống nhiễu (3D-DNR), LDC',
      '– Hỗ trợ chức năng phát hiện thông minh: Hàng rào ảo, Xâm nhập (phân biệt người và xe), SMD Plus',
      '– Tìm kiếm thông minh: Tìm kiếm nhanh sự kiện theo từng đối tượng (người, xe) khi kết hợp NVR',
      '– Hỗ trợ khe cắm thẻ nhớ 256GB, Tích hợp mic',
      '– Chuẩn tương thích ONVIF, hỗ trợ tên miền miễn phí SmartDDNS.TV, AutoRegister 2.0',
      '– Chuẩn chống nước IP67',
      '– Điện áp DC12V hoặc PoE',
      '– Nhiệt độ hoạt động: -40° C ~ +60° C',
      '– Chất liệu kim loại, kèm chân đế',
      '– Kích thước: 201.0 mm × 103.8 mm × 97.8 mm',
      '– Trọng lượng: 0.68 kg',
      '– Xuất xứ: Trung Quốc.',
    ],
    price: 20500000,
    rating: 5.0,
    brand: "Dahua",
    color: ['white'],
    material: ['kim loại', 'nhựa'],
    stockItems: 5,
    reviews: [
      {
        content: "To, Dài, Nạc",
        author: "Nhân cách 03",
        image: "/images/people/person.jpg",
        rating:4,
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/HFW2449M-S-B-PRO.png",
    ],
  },
  {
    id: 18,
    name: "Camera IP Wifi Bullet C1 4MP Smart Dual Light DAHUA DH-F4C-PV",
    category: "Camera WiFi",
    description:
      "DAHUA DH-F4C-PV thiết kế nhỏ gọn nhẹ, độ phân giải cao 4.0 megapixel, tầm quan sát xa 30m dùng trong nhà cho hình ảnh rõ nét. Hoạt động rất dễ dàng qua thiết bị di động (Android/iOS), cho phép người dùng hoạt động thoải mái.",
    aboutItem: [
      "Nhiều chế độ quan sát ban đêm, đáp ứng mọi nhu cầu",
      "Được trang bị công nghệ Chiếu sáng kép Thông minh – Chiếu sáng kép thông minh, đáp ứng nhu cầu của các tình huống khác nhau và ghi lại hình ảnh màu của các sự kiện quan trọng.",
      "Cung cấp cảnh báo tức thì để ngăn chặn xâm nhập",
      "Báo động âm thanh và ánh sáng cung cấp cảnh báo sớm có thể được gửi nhanh chóng, ngăn chặn hiệu quả mọi hành vi xâm nhập bất hợp pháp tiềm ẩn.",
      "Tập trung vào đối tượng, giảm chi phí nhân công",
      "Tính năng phát hiện người AI, tập trung vào cảnh báo khi có người,bạn sẽ không phải lo về cảnh báo giả và có thể giảm tối thiểu chi phí nhân công.",
      "Đáng tin cậy và Bền bỉ",
      "Với chỉ số bảo vệ IP67, có thể chịu được môi trường khắc nghiệt với mưa, sương mù hoặc bụi bẩn, đảm bảo hoạt động 24/7.",
    ],
    price: 9500000,
    rating: 4.9,
    brand: "Dahua",
    color: ['black'],
    material: ['kim loại', 'nhựa'],
    stockItems: 0,
    reviews: [
      {
        content: "Toẹt vời, đúng đỉnh",
        author: "Nhân cách C",
        image: "/images/people/person.jpg",
        rating:4,
        date: new Date(),
      },
    ],
    discount: 5,
    images: [
      "/images/products/camerawifi.png",
    ],
  },
];


